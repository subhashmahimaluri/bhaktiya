...
export class YexaaPanchangImpl {
  ...
  daysInMonth(m: number, y: number) {
    const g_days: number[] = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  ...
  sun(jd: number) {
    ...
    // perigee sun
    const pes = 101.220833 + (6189.03 * t) / 3600 + (1.63 * t2) / 3600 + (0.012 * t3) / 3600;
    // avg anomoly sun

    const ms = this.fix360(ls - pes + 180);
    const g =
      ms +
    ...
    const u = this.kepler(g, ex, 0.0000003);
    ...
    const u1 = (153.23 + 22518.7541 * t) * this.d2r;
    const u2 = (216.57 + 45037.5082 * t) * this.d2r;
    const u3 = (312.69 + 32964.3577 * t) * this.d2r;
    const u4 = (350.74 + 445267.1142 * t - 0.00144 * t2) * this.d2r;
    const u6 = (353.4 + 65928.71550000001 * t) * this.d2r;
    const u5 = (315.6 + 893.3 * t) * this.d2r;
    ...
    const r1 = (1.0000002 * (1 - ex * ex)) / (1 + ex * Math.cos(truanom * this.d2r));
    const rs = r1 + dr; // radius vector
  ...
  tithi(jd: number, n1: number, tzone: number, len: number) {
    const s_t: Record<string, number | Date> = {};
    let flag;
    let jdt = jd;
    ...
    for (let itit = n1; itit < n1 + 2; ++itit) {
      ...
      if (aspect === 360) {
        // For tithi 30's end (aspect = 360), we need to find when phase completes at 360°
        // This is the same as finding phase = 0° (next new moon), but within ~1 day
        // Don't jump to next lunar cycle (knv + 1), stay in current cycle
        // Use the iterative approach instead
        jdt = jd + 0.5; // Start from half a day ahead
        flag = 0; // Allow iteration to find the exact moment
      }
      while (flag < 1) {
        const Lsun0 = this.sun(jdt);
        const Lmoon0 = this.moon(jdt);
        const a = this.fix360(Lsun0 + aspect); // pt should be where luna
  ...
  nakshatra(jd: number, n_naksh: number, tzone: number) {
    ...
    for (let inak = n_naksh; inak < n_naksh + 2; ++inak) {
      const n1 = this.fix360((inak * 80) / 6); // co-ordinate start of nakshatra
      flag = 0;
      while (flag < 1) {
        const Lmoon0 = this.fix360(this.moon(jdt) + this.ayanamsa);
  ...
  yoga(jd: number, zyoga: number, tzone: number) {
    ...
    for (let iyog = 0; iyog < 2; ++iyog) {
      ...
      while (flag < 1) {
        this.sun(jdt);
        this.moon(jdt);
        const dmoonYoga = this.LmoonYoga + this.ayanamsa - 491143.07698973856;
        const dsunYoga = this.LsunYoga + this.ayanamsa - 36976.91240579201;
  ...
  dTime(jd: number) {
    ...
    this.calData(jd);
    const dgod = this.kyear + (this.kmon - 1) / 12 + (this.kday - 1) / 365.25;
  ...
  calData(jd: number) {
    ...
    const hh1 = (days - this.kday) * 24;
    const khr = Math.floor(hh1);
    let kmin = hh1 - khr;
    let ksek = kmin * 60;
    kmin = Math.floor(ksek);
    ksek = Math.floor((ksek - kmin) * 60);
    const s = new Date(this.kyear, this.kmon - 1, this.kday, khr, kmin, ksek, 0);
  ...
  weekDay(jd: number) {
    ...
    const wday = Math.floor(jdn - dn1);

    return wday;
  }

  lunarPhase(jd: number): number {
    const Lmoon = this.moon(jd); // Logitudinal Moon
    const Lsun = this.sun(jd); // Logitudinal Sun
