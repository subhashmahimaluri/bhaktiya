...
import { YexaaTithiCalculate } from './yexaaTithiCalculate';
...
export class YexaaPanchang {
  ...
  /**
   * Reverse tithi calculation: Given tithi.ino, masa.ino, lat, lng, year
   * Return the start and end times of that specific tithi in that specific masa
   *
   * @param tithiIno - Tithi index (0-29)
   * @param masaIno - Masa index (0-11, Amanta system)
   * @param lat - Latitude
   * @param lng - Longitude
   * @param year - Gregorian year (for reference)
   * @param height - Optional elevation in meters
   * @param preferLeapMonth - Whether to prefer leap month when multiple options exist
   * @param tzone - Optional timezone offset in hours (e.g., 5.5 for IST)
   * @returns Object with startTime and endTime as Date objects, or null if not found
   */
  ...
  calendarAtPradosha(dt: Date, lat: number, lng: number, height?: number) {
    return this.yexaaCalendar.calendarAtPradosha(this.yexaaLocalConstant, dt, lat, lng, height);
  }

  calendarAtSunset(dt: Date, lat: number, lng: number, height?: number) {
    return this.yexaaCalendar.calendarAtSunset(this.yexaaLocalConstant, dt, lat, lng, height);
  }
  ...
  private matchesPanchangamCriteria(
    calculated: Record<string, Record<string, string | number | Date>>,
    calendar: Record<string, Record<string, string | number | Date | boolean>>,
    normalizedMasa: string,
    ...
    const moonMasa = calendar.MoonMasa?.name_TE || '';
    const solarMasa = calendar.Masa?.name_TE || '';

    // If both are empty, this might be a transition period - let's be more flexible
    const availableMasas = [moonMasa, solarMasa].filter(m => m && String(m).trim());
    ...
    if (availableMasas.length > 0) {
      ...
      for (const masa of availableMasas) {
        const normalizedCalculatedMasa = this.normalizeName(String(masa));
        if (normalizedCalculatedMasa === normalizedMasa) {
          masaMatches = true;
          actualMasaUsed = String(masa);
          break;
        }
      }
    } else {
    ...
    // Check paksha match
    const calculatedPaksha = calculated.Paksha?.name_TE || '';
    const normalizedCalculatedPaksha = this.normalizeName(String(calculatedPaksha));

    const pakshaMatches = normalizedCalculatedPaksha === normalizedPaksha;

    // Check tithi match
    const calculatedTithi = calculated.Tithi?.name || '';
    const normalizedCalculatedTithi = this.normalizeName(String(calculatedTithi));
  ...
  debugFindTithiOccurrences(
    ...
    while (currentDate <= endDate) {
      try {
        ...
        const calculatedMasa = calendar.MoonMasa?.name_TE || '';
        const calculatedPaksha = calculated.Paksha?.name_TE || '';
        const calculatedTithi = calculated.Tithi?.name || '';

        // Store ALL tithis that match our target masa
        if (this.normalizeName(String(calculatedMasa)) === normalizedMasa) {
          results.push({
            date: new Date(currentDate),
            masa: String(calculatedMasa),
            paksha: String(calculatedPaksha),
            tithi: String(calculatedTithi),
          });
        }
      } catch (error) {
