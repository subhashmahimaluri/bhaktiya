...
export default function StotraEditor({ stotraId }: StotraEditorProps) {
  ...
  const stotraEditorRef = useRef<SummernoteEditorRef>(null);
  const meaningEditorRef = useRef<SummernoteEditorRef>(null);
  const faqEditorRef = useRef<SummernoteEditorRef>(null);
  ...
  const loadStotra = async () => {
    ...
    try {
      ...
      newFormData.faq[currentLocale as keyof typeof newFormData.faq] = stotra.faq
        ? decodeHtmlEntities(stotra.faq)
        : '';
  ...
  const handleFieldChange = (field: string, value: string) => {
    if (field === 'stotraTitle') {
      ...
      setFormData(prev => ({
        ...prev,
        faq: { ...prev.faq, [currentLocale]: value },
      }));
  ...
  const handleFaqEditorChange = (data: string) => {
    setFormData(prev => ({
      ...prev,
      faq: { ...prev.faq, [currentLocale]: data },
    }));
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.stotraTitle?.trim()) {
      newErrors.push('Stotra Title (Common for all languages) is required');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Early validation check - prevent submission if mandatory fields are empty
    if (!formData.stotraTitle?.trim()) {
      setErrors(['Stotra Title (Common for all languages) is required']);
      return;
    }
    ...
    try {
      const stotraData = {
        ...
        faq: formData.faq[currentLocale as keyof typeof formData.faq] || null,
      ...
      const errorMessage = `Failed to save stotra: ${error instanceof Error ? error.message : 'Unknown error'}`;
  ...
  const handleSaveAsDraft = async () => {
    // Check if stotraTitle is provided before saving
    if (!formData.stotraTitle?.trim()) {
      setErrors(['Stotra Title (Common for all languages) is required']);
      return;
    }
      ...
      <Form onSubmit={handleSubmit}>
        <Row>
          <Col lg={8}>
              ...
              <Card.Body>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    required
                    className={!formData.stotraTitle?.trim() ? 'is-invalid' : ''}
                  />
                  {!formData.stotraTitle?.trim() && (
                    <Form.Control.Feedback type="invalid">
                      Stotra Title is required
            ...
            {/* FAQ Editor - Separate Section */}
            <Card className="stotra-faq-card mb-3">
              <Card.Header className="bg-light">
                <h6 className="mb-0">
                  <i className="bi bi-question-circle me-2"></i>
                  FAQ Section ({currentLocale.toUpperCase()})
                </h6>
                <small className="text-muted">
                  Add Frequently Asked Questions using the details/summary HTML structure
                </small>
              </Card.Header>
              <Card.Body className="stotra-faq-editor">
                <SummernoteEditor
                  id="stotra-faq-editor"
                  ref={faqEditorRef}
                  data={formData.faq[currentLocale as keyof typeof formData.faq] || ''}
                  onChange={handleFaqEditorChange}
                />
              </Card.Body>
            </Card>
          </Col>
              ...
              <Card.Body>
                ...
                <div className="d-grid gap-2">
                  <Button
                    type="submit"
                    variant="primary"
                    disabled={saving || !formData.stotraTitle?.trim()}
                  >
