...
interface StotraCardProps {
  ...
  categoryContext?: 'ashtothram' | 'sahasranamavali' | 'sahasranamam' | 'bhajans' | 'bhakthisongs' | 'default'; // Updated to include sahasranamavali and sahasranamam
}
...
export default function StotraCard({
  ...
  const getHref = () => {
    ...
    if (categoryContext === 'bhajans') {
      return `/bhajans/${stotra.canonicalSlug}`;
    }
    if (categoryContext === 'bhakthisongs') {
      return `/bhakthi-songs/${stotra.canonicalSlug}`;
    }
