...
interface CommentSectionProps {
  contentType: 'stotra' | 'article' | 'festival' | 'eclipse';
  canonicalSlug: string;
}
        ...
        <div className={`card border-0 shadow-sm ${isEditing ? 'border-primary' : ''}`}>
          <div className="card-body">
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-start mb-2">
              <div className="d-flex align-items-center commet-username mb-md-0 mb-2">
              ...
              <div className="d-flex align-items-center commet-actions">
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/comments/${canonicalSlug}?lang=${locale}&limit=20&offset=0`,
        {
          signal: abortController.signal, // Add abort signal
        }
      );
    ...
    try {
      ...
      if (decoded && decoded.exp) {
        ...
        if (expiresAt < now + bufferTime) {
          ...
          console.warn('Token expiration:', new Date(expiresAt).toISOString());
