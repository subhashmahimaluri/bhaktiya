...
export default function AutoComplete({ onLocationSelect, selectedLocation }: Props) {
  ...
  useEffect(() => {
    ...
    if (!isEmpty(data)) {
      ...
      // If local storage data doesn't have offset, we need to fetch it from cityMapNew.json
      if (!localData.offset && items.length > 0) {
        const cityData = items.find(
          item => item.city === localData.city && item.country === localData.country
        );
        if (cityData) {
          // Update local storage with complete data including offset
          const completeLocationData = {
            ...localData,
            offset: cityData.offset,
          };
          reactLocalStorage.setObject('LOCATION', completeLocationData);
  ...
  const setValues = (item: CityData) => {
    ...
    // Store location in context with offset for API calls
    const locationData = {
      city: item.city,
      ...
      offset: item.offset, // Include offset from cityMapNew.json
    };
