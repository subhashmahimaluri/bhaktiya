...
export default function Sahasranamam() {
  ...
  const { title, description } = getMetaDataByPath('/sahasranamam', locale);
  ...
  const fetchStotras = async (page: number = 1, reset: boolean = false) => {
    try {
      ...
      const apiUrl = `/api/stotras?lang=${locale}&page=${page}&limit=${ITEMS_PER_PAGE}&categoryId=${CATEGORY_IDS.SAHASRANAMAM}`;
      ...
      <Row className="mt-25 inner-page py-5">
        <Col xl="8" lg="8" md="12" className="mt-5 pt-5">
          <div className="left-container shadow-1 panchangam-block px-md-10 bg-white px-5 py-3 text-black">
            <h1 className="text-center">{t.stotra.sahasranama_stotra}</h1>
            <p className="text-center">{t.stotra.sasranama_stotra_desc}</p>
            ...
            {!loading && !error && stotras.length === 0 && (
              <div className="py-4 text-center">
                <p className="text-muted">No Sahasranamam available at the moment.</p>
              </div>
            )}
          </div>
        </Col>
