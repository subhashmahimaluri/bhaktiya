...
export default function EditCategoryPage({ userRoles, categoryId }: EditCategoryPageProps) {
  ...
  const loadCategory = async () => {
    try {
      ...
      setFormData({
        name: {
          en: foundCategory.name?.en || '',
          te: foundCategory.name?.te || '',
        },
        slug: {
          en: foundCategory.slug?.en || '',
          te: foundCategory.slug?.te || '',
        },
        description: {
          en: foundCategory.description?.en || '',
          te: foundCategory.description?.te || '',
        },
        taxonomy: foundCategory.meta?.taxonomy || 'type',
        order: foundCategory.order || 0,
        isActive: foundCategory.isActive !== false,
      });
    } catch (error) {
      console.error('Error loading category:', error);
      setErrors([error instanceof Error ? error.message : 'Failed to load category']);
    } finally {
      setLoading(false);
    }
  };
      ...
      <Layout>
        ...
        <Container className="py-5">
          <div className="text-center">
            <h2>Access Denied</h2>
            <p>You don&apos;t have permission to edit categories.</p>
  ...
  const handleNameChange = (lang: 'en' | 'te', value: string) => {
    setFormData(prev => ({
      ...
      // Auto-generate slug for English name only if it's currently empty or matches the auto-generated pattern
      ...(lang === 'en' &&
        (!prev.slug.en || prev.slug.en === generateSlug(prev.name.en)) && {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.details || `Failed to update category: ${response.status}`);
      }
      ...
      setErrors([error instanceof Error ? error.message : 'Failed to update category']);
    } finally {
      setSaving(false);
    }
  };
        ...
        <Row className="mb-4">
          <Col>
            ...
            <p className="text-muted">
              Update the category details. Changes will affect all content using this category.
            </p>
          </Col>
          <Col xs="auto">
          ...
          <Row>
                  ...
                  <Row>
                    <Col md={6}>
                      <Form.Group className="mb-3">
                        ...
                        />
                        <Form.Text className="text-muted">
                          Use lowercase, hyphens only. Changing this may break existing URLs.
                        </Form.Text>
                      </Form.Group>
                    </Col>
                    <Col md={6}>
            ...
            <Col lg={4}>
              ...
              <Card className="bg-light mb-4">
                <Card.Header>
                  <h6 className="mb-0">📋 Category Info</h6>
                </Card.Header>
                <Card.Body>
                  <div className="small">
