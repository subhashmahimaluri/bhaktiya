import hinduFestivalsData from './data/hinduFestivals.json';
...
export function getTithiDates(year: number, tithiIno: number, lat: number, lng: number) {
  ...
  while (currentDate < endDate) {
    try {
      ...
      for (const checkJd of checkPoints) {
      ...
      if (foundTithi) {
        ...
        if (tithiTiming && tithiTiming.start && tithiTiming.end) {
          ...
          let masaNameEnIn = panchangamCalendar.MoonMasa.name_TE;
          ...
          // Get solar raasi at the new moon
          const solarLongAtNewMoon = yexaaPanchangImpl.fix360(
            yexaaPanchangImpl.sun(newMoonJd) + yexaaPanchangImpl.calcayan(newMoonJd)
          );

          // Calculate n_maasa (1-12) using solar raasi
          const solarRaasi = Math.ceil(solarLongAtNewMoon / 30); // 1-12
          const n_maasa = solarRaasi;

          // Calculate Amanta Masa index (0-11) for masa name lookup
          // Amanta Masa = n_maasa - 1
          let amantaMasaIndex = n_maasa - 1;
          if (amantaMasaIndex < 0) {
            amantaMasaIndex += 12;
          }
          amantaMasaIndex = amantaMasaIndex % 12;

          masaIno = amantaMasaIndex;
          masaName = yexaaConstant.Masa.name[masaIno];
          masaNameEnIn = yexaaConstant.Masa.name_TE[masaIno];
          ...
          const result = {
            tithi: {
              ...
              name_TE: yexaaConstant.Tithi.name_TE[tithiIno],
              ino: tithiIno,
            ...
            paksha: {
              ...
              name_TE: yexaaConstant.Paksha.name_TE[n_paksha],
            },
            masa: {
              ino: masaIno,
              name: masaName,
              name_TE: masaNameEnIn,
...
function getCalendarTithi(jd: number, yexaaPanchangImpl: YexaaPanchangImpl) {
  const moonPhase = yexaaPanchangImpl.lunarPhase(jd);
...
// Function to get festival information by name
export function getFestivalInfo(festivalName: string) {
  if (!festivalName) return null;

  const searchTerm = festivalName.toLowerCase().trim();

  // Search for the festival in the data
  const festival = hinduFestivalsData.find(
    (item: {
      ...
      // Check if the search term matches any of the festival name fields
      return (
        item.Festival_name.toLowerCase().includes(searchTerm) ||
        item.Festival_en.toLowerCase().includes(searchTerm) ||
        item.Festival_ta.toLowerCase().includes(searchTerm) ||
        item.Festival_te.toLowerCase().includes(searchTerm) ||
        item.Festival_ka.toLowerCase().includes(searchTerm) ||
        item.Festival_hi.toLowerCase().includes(searchTerm) ||
        item.Festival_ml.toLowerCase().includes(searchTerm)
      );
    }
  );
...
export async function getFestivalDates(
  year: number,
  lunarMonth: number,
  tithiIno: number,
  lat: number,
  ...
  // Re-export from the separate module to avoid circular imports
  const festivalModule = await import('./getFestivalDates');
  return festivalModule.getFestivalDates(year, lunarMonth, tithiIno, lat, lng);
}
