export class YexaaSunMoonTimer {
  ...
  sunTimer(date: Date, lat: number, lng: number, height?: number) {
    const calS = this.calculatSunTimer(date, lat, lng, height);
    ...
    for (let i = 0; i < this.times.length; i += 1) {
      ...
      const h0 = (time[0] + calS.dh) * this.rad;

      const Jset = this.getSetJ(h0, calS.lw, calS.phi, calS.dec, calS.n, calS.M, calS.L);
      const Jrise = calS.Jnoon - (Jset - calS.Jnoon);
  ...
  eclipticLongitude(M: number): number {
    const C = this.rad * (1.9148 * Math.sin(M) + 0.02 * Math.sin(2 * M) + 0.0003 * Math.sin(3 * M)), // equation of center
