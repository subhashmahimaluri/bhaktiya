...
// Type definitions for Panchang calculations
...
export class YexaaPanchang_SuryaSiddhantam {
  ...
  calcayan(jd: number): number {
    ...
    const daysSinceEpoch = jd - this.config.epoch.julianDay;
    ...
    const ayanamsa = (yearsSinceEpoch * this.config.ayanamsa.ratePerYear) / 3600.0;
  ...
  dTime(jd: number): number {
    const year = this.calData(jd).year;
  ...
  sun(jd: number): number {
    ...
    let correction = 0;
    for (const sunCorr of this.config.corrections.sunCorrections) {
    ...
    const trueLongitude = meanLongitude + correction;
  ...
  moon(jd: number): number {
    ...
    const sunMeanLong =
      this.config.initialPositions.sun.longitude +
      daysSinceEpoch * this.config.meanMotions.sun.dailyMotion;
    const meanElongation = meanLongitude - sunMeanLong;
    ...
    let correction = 0;
    for (const moonCorr of this.config.corrections.moonCorrections) {
  ...
  lon2dms(longitude: number): string {
    ...
    const sec = Math.floor(((longitude - deg) * 60 - min) * 60);
   ...
   */
  tithi(jd: number, tithiNum: number, tzone: number, angularStep: number = 12): TimingResult {
    const startJD = jd;
    const targetAngle = tithiNum * angularStep;
    const nextTargetAngle = (tithiNum + 1) * angularStep;

    // Find tithi start time
    const tithiStartJD = this.findEventTime(startJD - 2, 'tithi', targetAngle);

    // Find tithi end time
    const tithiEndJD = this.findEventTime(tithiStartJD, 'tithi', nextTargetAngle);
   ...
   */
  nakshatra(jd: number, nakshatraNum: number, tzone: number): TimingResult {
    const startJD = jd;
    const targetAngle = nakshatraNum * (360 / 27); // 13.33 degrees per nakshatra
    const nextTargetAngle = (nakshatraNum + 1) * (360 / 27);

    // Find nakshatra start time
    const nakshatraStartJD = this.findEventTime(startJD - 2, 'nakshatra', targetAngle);

    // Find nakshatra end time
    const nakshatraEndJD = this.findEventTime(nakshatraStartJD, 'nakshatra', nextTargetAngle);
   ...
   */
  yoga(jd: number, yogaVal: number, tzone: number): TimingResult {
    const startJD = jd;
    const targetAngle = (yogaVal * 360) / 27; // 13.33 degrees per yoga
    const nextTargetAngle = ((yogaVal + 1) * 360) / 27;

    // Find yoga start time
    const yogaStartJD = this.findEventTime(startJD - 2, 'yoga', targetAngle);

    // Find yoga end time
    const yogaEndJD = this.findEventTime(yogaStartJD, 'yoga', nextTargetAngle);
  ...
  private findEventTime(startJD: number, eventType: string, targetAngle: number): number {
    let jd = startJD;
    const tolerance = 1 / 1440; // 1 minute precision
    ...
    while (iterations < maxIterations) {
      ...
      const timeStep = angleDiff / (this.skor || 13.176358); // Use Moon's mean motion as fallback
      jd += timeStep;
   ...
   */
  computePanchang(params: { date: Date; lat: number; lon: number; tz: string }): PanchangResult {
    ...
    const jd = this.mdy2julian(month, day + hour / 24, year);

    // Calculate ayanamsa
    this.calcayan(jd);

    // Calculate celestial positions
    const sunLong = this.sun(jd);
    const moonLong = this.moon(jd);

    // Calculate basic elements
    const tithiNum = this.getTithi(moonLong, sunLong);
    const nakshatraNum = this.getNakshatra(moonLong);
    const yogaVal = this.getYoga(sunLong, moonLong);
    const karanaData = this.getKarana(moonLong, sunLong);

    // Calculate timing details
    const tithiTiming = this.tithi(jd, tithiNum, 5.5); // IST timezone
    const nakshatraTiming = this.nakshatra(jd, nakshatraNum, 5.5);
    const yogaTiming = this.yoga(jd, yogaVal, 5.5);
    const karanaTiming = this.tithi(jd, karanaData[1], 5.5, 6); // Karana is 6 degrees

    return {
      tithi: {
        number: tithiNum,
        name: this.yexaaConstant.Tithi.name_TE[tithiNum],
        start: tithiTiming.start,
        end: tithiTiming.end,
      },
      nakshatra: {
        number: nakshatraNum,
        name: this.yexaaConstant.Nakshatra.name_TE[nakshatraNum],
        start: nakshatraTiming.start,
        end: nakshatraTiming.end,
      },
      yoga: {
        number: yogaVal,
        name: this.yexaaConstant.Yoga.name_TE[yogaVal],
        start: yogaTiming.start,
        end: yogaTiming.end,
      },
      karana: {
        number: karanaData[0],
        name: this.yexaaConstant.Karna.name_TE[karanaData[0]],
  ...
  private getNakshatra(moonLong: number): number {
    const siderealMoon = this.fix360(moonLong + this.ayanamsa);
    return Math.floor((siderealMoon * 27) / 360);
  }

  private getYoga(sunLong: number, moonLong: number): number {
    const sum = this.fix360(sunLong + moonLong + 2 * this.ayanamsa);
   ...
   */
  compareWithDrik(date: Date, lat: number, lon: number): ComparisonResult {
   ...
   */
  private getDrikCalculation(
    date: Date,
    lat: number,
    lon: number
  ): { note: string; placeholder: boolean } {
